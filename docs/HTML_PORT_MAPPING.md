...
| First person pohyb a mouse look | `FirstPersonController.cs` | DONE | WASD, myš, skok, sprint a vývojářský fly mode. |
| Pevný ostrov / ground collider | `WorldGenerator.cs` | DONE | Ostrov byl zvětšen na skutečný poloměr 74 m a má samostatný záložní collider. |
| Bezpečný spawn | `WorldGenerator.PlayerSpawn`, `GameManager.Respawn()` | DONE | Spawn je 2,2 m nad povrchem. |
| Failsafe pod `y = -20` | `FirstPersonController.Update()` | DONE | Automatický respawn na bezpečný bod. |
| Rust-like HUD | `HUDController.cs`, `RuntimeUI.cs` | DONE | Nativní `Canvas`, tmavé panely, opacity, kotvení pro 1920x1080 a škálování. |
...
| Bleeding / cold / wet / radiation alerts | `HUDController.UpdateStatus()` | DONE | Výstrahy se objeví pouze při problému. |
| Hlad, žízeň a damage over time | `PlayerStats.cs` | DONE | Průběžný úbytek a poškození při kritických stavech. |
...
| Optimalizovaný pickup | `DroppedItemWorldSpawner.cs` | DONE | Object pool, cache materiálů a aktualizace existujících UI slotů. |
...
| Private dev terminal | `DevConsoleUI.cs` | PARTIAL | F10/` a příkazy give, god, fly, save/load/report/clear. HTML weather/time/spawn/kill příkazy chybí. |
