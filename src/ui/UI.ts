...
export class UI {
  ...
  setSettings(settings: Settings): void {
    ...
    for(const name of ['invertY','headBob','cameraShake','motionBlur','shadows','showCompass','showFps','showTutorialHints','postProcessing','ambientOcclusion','bloom','telemetryPerformance','telemetryPlayer','telemetryCamera','telemetryWorld'] as const)this.root.querySelectorAll<HTMLElement>(`[data-toggle="${name}"]`).forEach(button=>button.classList.toggle('active',String(settings[name])===button.dataset.value));
  ...
  private bindEvents(): void {
    ...
    this.root.addEventListener('click',event=>{
      ...
      if(target.dataset.toggle){const name=target.dataset.toggle as 'invertY'|'headBob'|'cameraShake'|'motionBlur'|'shadows'|'showCompass'|'showFps'|'showTutorialHints'|'postProcessing'|'ambientOcclusion'|'bloom'|'telemetryPerformance'|'telemetryPlayer'|'telemetryCamera'|'telemetryWorld';this.settings[name]=target.dataset.value==='true';this.actions.settings({...this.settings,keybinds:{...this.settings.keybinds}});this.setSettings(this.settings);}
