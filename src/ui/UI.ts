...
export class UI {
  ...
  setDeathContext(context:{atBedroll:boolean;lostPack:boolean;distance?:number}):void{
    ...
    button.innerHTML=`${context.atBedroll?(cs?'ZNOVUZRODIT U SPACÍHO VAKU':'RESPAWN AT SLEEPING ROLL'):(cs?'ZNOVUZRODIT NA POBŘEŽÍ':'RESPAWN ON SHORE')} ${chevron}`;
  }

  playRespawnFade():void{this.root.classList.remove('respawn-fade');void this.root.offsetWidth;this.root.classList.add('respawn-fade');window.setTimeout(()=>this.root.classList.remove('respawn-fade'),900);}
