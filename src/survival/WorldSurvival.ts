...
export class IslandMap {
  ...
  get isOpen(){return !this.root.hidden;}show(){this.root.hidden=false;}close(){this.root.hidden=true;}update(p:Vec3,yaw:number,waypoint?:{x:number;z:number},packs:Vec3[]=[]){if(!this.isOpen)return;const c=this.ctx;c.drawImage(this.base,0,0);if(waypoint){c.strokeStyle='#e3a877';c.lineWidth=3;c.strokeRect(waypoint.x+352,waypoint.z+352,16,16);}for(const pack of packs){c.save();c.translate(pack.x+360,pack.z+360);c.rotate(Math.PI/4);c.fillStyle='#d88955';c.strokeStyle='#f1d0a0';c.lineWidth=2;c.fillRect(-6,-6,12,12);c.strokeRect(-6,-6,12,12);c.restore();c.fillStyle='#f1d0a0';c.font='bold 11px sans-serif';c.fillText('LOST PACK',pack.x+371,pack.z+364);}c.save();c.translate(p.x+360,p.z+360);c.rotate(-yaw);c.fillStyle='#f0f4e4';c.beginPath();c.moveTo(0,-10);c.lineTo(-6,7);c.lineTo(6,7);c.closePath();c.fill();c.restore();}
