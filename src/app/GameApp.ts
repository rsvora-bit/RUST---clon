...
import {createStation,isPlaceableStationKind,MAX_PLAYER_STATIONS,STATIONS,STATION_KINDS,transfer,takeAll,type Station,type StationKind} from '../survival/stations';
...
export class GameApp {
  ...
  private async makeWorld(seed:number,saved?:GameState){
    ...
    this.environment=new Environment(this.scene,seed,saved ? saved.worldGeneration ?? 1 : 4,true);
  ...
  private safeDeathPosition():Vec3{
    const p=this.simulation.state.player.position,valid=[p.x,p.y,p.z].every(Number.isFinite)&&Math.abs(p.x)<=360&&Math.abs(p.z)<=360;
  ...
  private respawnCandidate(origin:Vec3,ignoreStation?:string):Vec3|null{
    if(![origin.x,origin.z].every(Number.isFinite)||Math.abs(origin.x)>360||Math.abs(origin.z)>360)return null;const terrain=this.environment.heightAt(origin.x,origin.z);if(terrain<.12||this.environment.terrain.slopeAt(origin.x,origin.z)>.7)return null;const y=Math.max(terrain+.06,origin.y);
