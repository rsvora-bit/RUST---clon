...
export class GameApp {
  ...
  private last=0;private accumulator=0;private uiTimer=0;private elapsed=0;private autoSave=0;private cooldown=0;private fps=60;private frameMs=16.7;private timeMultiplier=1;private loading=false;private leftDown=false;private loopStarted=false;private loopMode:'vsync'|'uncapped'|null=null;private frameChannel:MessageChannel|null=null;private knownStructures=new Map<string,boolean|undefined>();private rainBarrels:THREE.Group[]=[];private flyMode=false;private godMode=false;
  ...
  private async makeWorld(seed:number,saved?:GameState){
    ...
    this.registerNodes();this.createWaterSources();this.syncStructures();this.syncWorldItems();this.syncHeld();this.applySettings(this.settings);this.player.tick(1/60,this.simulation.state,false);this.accumulator=0;this.autoSave=0;this.building=false;this.candidate=null;
  ...
  private syncStructures(){
    ...
    for(const s of state.structures){if(!this.knownStructures.has(s.id)||this.knownStructures.get(s.id)!==s.open){this.physics.setStructure(s.id,this.structures.boxes(s));this.knownStructures.set(s.id,s.open);}
      if(s.pieceType==='door'&&!this.interactions.entries.has(s.id)){this.interactions.register({id:s.id,kind:'door',object:this.structures.objects.get(s.id)!,position:()=>s.position,enabled:()=>true,info:()=>({title:'Timber door',action:s.open?'CLOSE':'OPEN',key:keyLabel(this.settings.keybinds.interact)}),interact:()=>{if(this.simulation.toggleDoor(s.id)){this.syncStructures();this.audio.play('door');}}});}}
  }
