...
export class GameApp {
  ...
  private installDevAPI(){
    ...
    (window as unknown as {__TIDELAND:unknown}).__TIDELAND={
      ...
      worldArt:()=>({horizonLayers:this.environment.atmosphere.horizon.children.length,horizonTriangles:this.environment.atmosphere.horizon.children.reduce((n,m)=>n+((m as THREE.Mesh).geometry.index?.count??0)/3,0),trees:this.environment.nodes.filter(n=>n.kind==='tree').map(n=>({id:n.id,position:{...n.position},species:this.environment.nodeObjects.get(n.id)?.userData.species,climate:this.environment.terrain.climateAt(n.position.x,n.position.z)}))}),
