...
await page.waitForFunction(()=>window.__TIDELAND.getScreen()==='playing',{timeout:150000});await page.waitForTimeout(800);
check('New Game',await page.evaluate(()=>window.__TIDELAND.snapshot().structures.length===0));
// This construction fixture was authored for the protected generation-1 clearing.
// Gen5 and its new routes are covered by world-overhaul/world-art QA separately.
await page.evaluate(()=>{const a=window.__TIDELAND,g=a.sim();g.state.worldGeneration=1;delete g.state.worldRevision;delete g.state.progression;g.state.player.position={x:28,y:6.1,z:212};a.save();});await page.reload();await page.waitForFunction(()=>window.__TIDELAND);await page.locator('.loading-screen').waitFor({state:'hidden',timeout:150000});await page.locator('[data-action="continue"]').click();await page.waitForFunction(()=>window.__TIDELAND?.getScreen()==='playing');
check('Legacy construction fixture loads generation 1',await page.evaluate(()=>window.__TIDELAND.world().generation===1));
...
await page.evaluate(()=>window.__TIDELAND.dev('spawn'));await page.waitForFunction(()=>window.__TIDELAND.physics().grounded&&window.__TIDELAND.cameraDebug().jumpCooldown<=0,null,{timeout:5000});await page.keyboard.press('Space',{delay:60});await page.waitForFunction(()=>!window.__TIDELAND.physics().grounded,null,{timeout:2000});check('Jump leaves ground',true);await page.waitForFunction(()=>window.__TIDELAND.physics().grounded,null,{timeout:5000});
await page.keyboard.down('ControlLeft');await page.waitForFunction(()=>window.__TIDELAND.cameraDebug().crouching,null,{timeout:2000});check('Crouch posture responds',true);await page.keyboard.up('ControlLeft');
...
await visibleSlot(9).dispatchEvent('dragstart',{dataTransfer:await page.evaluateHandle(()=>new DataTransfer())});await page.keyboard.press('Tab');await page.keyboard.press('Tab');await page.evaluate(()=>{const g=window.__TIDELAND.sim();g.addItem('fiber',12);g.addItem('wood',25)});await page.waitForTimeout(200);check('Close inventory during drag clears drag styling',await page.locator('.dragging').count()===0);await shot('inventory-720');await page.locator('[data-category="building"]:visible').click();await page.locator('[data-recipe="plan"]:visible').click();await page.locator('[data-action="craft"]:visible').click();await page.waitForTimeout(100);check('Craft button queues selected recipe',await page.evaluate(()=>window.__TIDELAND.snapshot().craftQueue.some(j=>j.recipeId==='plan')));
...
const base=await page.evaluate(async()=>{const a=window.__TIDELAND,g=a.sim(),{getSockets}={getSockets:window.__TIDELAND.sockets};g.state.craftQueue=[];g.state.inventory=Array(30).fill(null);for(const id of ['wood','stone','metal'])g.addItem(id,6000);g.addItem('plan',1);const failures=[];
...
for(const q of ['LOW','MEDIUM','HIGH']){await page.locator(`[data-preset="${q.toLowerCase()}"]`).click();await page.keyboard.press('Escape');await page.keyboard.press('Escape');await page.evaluate(()=>{const a=window.__TIDELAND;a.dev('spawn');a.lookAt({x:28,y:6,z:155})});await page.waitForTimeout(900);await shot(`world-${q.toLowerCase()}`);console.log('PRESET',q,await page.evaluate(()=>window.__TIDELAND.stats()));await page.keyboard.press('Escape');await page.getByRole('button',{name:/SETTINGS/}).filter({visible:true}).click();}
